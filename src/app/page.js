...
import { FaExclamationCircle } from "react-icons/fa"; // Example for Custom; replace as needed
...
const socialLinks = [
  {
    ...
    label: "GitHub",
    icon: (
      <FaGithub className="w-6 h-6 sm:w-8 sm:h-8 text-teal-300 hover:text-teal-200 transition-transform duration-300 hover:scale-110" />
    ),
  },
  {
    ...
    label: "LinkedIn",
    icon: (
      <FaLinkedin className="w-6 h-6 sm:w-8 sm:h-8 text-blue-400 hover:text-blue-300 transition-transform duration-300 hover:scale-110" />
    ),
  },
  {
    ...
    label: "Custom",
    icon: (
      <FaExclamationCircle className="w-6 h-6 sm:w-8 sm:h-8 text-amber-400 hover:text-amber-300 transition-transform duration-300 hover:scale-110" />
    ),
  },
];
...
export default function HomePage() {
  return (
   <div className="min-h-screen bg-[#0f172a] ring-2 ring-black flex items-center justify-center">
  <main className="w-full flex flex-col md:flex-row items-center gap-10 px-6 py-12 max-w-7xl">
        ...
        {socialLinks.map(({ href, label, icon }) => (
          <Link key={href} href={href} aria-label={label} className="focus:outline-none focus-visible:ring-2 rounded">
            ...
            {React.cloneElement(icon, {
              className: icon.props.className.replace(/text-[^ ]+/, {
                GitHub: "text-teal-300 hover:text-teal-200",
                LinkedIn: "text-blue-400 hover:text-blue-300",
                Custom: "text-amber-400 hover:text-amber-300",
              }[label] || ""),
            })}
          </Link>
        ))}
